...
from utils.sample_utils import load_model
def load_models(
    pretrained_model_name_or_path: str,
...
) -> tuple[CLIPTokenizer, CLIPTextModel, UNetModel, SchedulerMixin,]:
    if pretrained_model_name_or_path.endswith(
        ".ckpt"
    ) or pretrained_model_name_or_path.endswith(".safetensors"):
        tokenizer, text_encoder = load_diffusers_model(
            "CompVis/stable-diffusion-v1-4", v2=v2, weight_dtype=weight_dtype
        )
        print("working 1")
        from omegaconf import OmegaConf
        config = OmegaConf.load(config_path)
        unet, _, _ = load_model(config, pretrained_model_name_or_path)
        print("working 2")
    else:  # diffusers
