...
from models.modules.openaimodel3d import UNetModel
...
class LoRAModule(nn.Module):
    ...
    def __init__(
        ...
        if "Linear" in org_module.__class__.__name__:
            ...
            self.lora_down = nn.Linear(in_dim, lora_dim, bias=False, dtype=torch.float32)
            self.lora_up = nn.Linear(lora_dim, out_dim, bias=False, dtype=torch.float32)
            ...
            if("Conv3d" in org_module.__class__.__name__):
                self.lora_down = nn.Conv3d(
                    in_dim, self.lora_dim, kernel_size, stride, padding, bias=False, dtype=torch.float32
                )
                self.lora_up = nn.Conv3d(self.lora_dim, out_dim, (1, 1), (1, 1), bias=False, dtype=torch.float32)
            else:
                self.lora_down = nn.Conv2d(
                    in_dim, self.lora_dim, kernel_size, stride, padding, bias=False, dtype=torch.float32
                )
                self.lora_up = nn.Conv2d(self.lora_dim, out_dim, (1, 1), (1, 1), bias=False, dtype=torch.float32)              

        # with open("output1.txt",'a') as f:
        #     print(self.lora_up.weight.data,file=f)
    ...
    def forward(self, x):
        with open('output5.txt','a') as f:
            t = self.lora_down(x)
            print(x.requires_grad,file=f)
            print(t.requires_grad,file=f)
            print('\nnext\n',file=f)
        with open("output2.txt",'a') as f:
            # print(self.org_forward(x),file=f)
            # print("next",file=f)
            # print(self.lora_up(self.lora_down(x))*1000000000000,file=f)
            # t = self.lora_down(x)
            for name, param in self.lora_down.named_parameters():
                print(f"Parameter: {name}, requires_grad: {param.requires_grad}",file=f)
            # print(t,file=f)
            # print(t.requires_grad,file=f)
            # print(self.lora_up.weight.data,file=f)
            # print("next",file=f)
            # print(self.lora_down.weight.data,file=f)
            # print('\n\n',file=f)
        return (
            self.org_forward(x)
    ...
    # def forward(self, x):
    # # Get the data type from org_forward
    #     org_output = self.org_forward(x)
    #     # Ensure input x and all operations use the same dtype
    #     # x = x.to(org_output.dtype)
    #     with open('output.txt','w') as file:
    #         print(org_output.dtype, x.dtype, file=file)
    #     lora_down_output = self.lora_down(x)


    #     # lora_up_output = self.lora_up(lora_down_output).to(org_output.dtype)
        
    #     # Return the final output
    #     return org_output # + lora_up_output * self.multiplier * self.scale
        ...
        for name, module in root_module.named_modules():
            ...
            if module.__class__.__name__ in target_replace_modules:
                for child_name, child_module in module.named_modules():
                    if child_module.__class__.__name__ in ["Linear", "Conv2d", "Conv3d", "LoRACompatibleLinear", "LoRACompatibleConv"]:
