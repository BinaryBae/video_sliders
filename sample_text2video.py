...
from utils.dist_utils import setup_dist, gather_data
from sample_utils import (load_model, 
...
def sample_text2video(model, prompt, n_samples, batch_size,
                      ...
                      ddim_steps=150, eta=1.0, cfg_scale=7.5, 
                      decode_frame_bs=1,
    ...
    network = LoRANetwork(
        ...
        alpha=-500,
        train_method="full",
    ).to(torch.device("cuda:0"), dtype=config_util.parse_precision("float32"))
    lora_weight = 'models/ageslider_alpha1.0_rank4_full/ageslider_alpha1.0_rank4_full_last.pt'
